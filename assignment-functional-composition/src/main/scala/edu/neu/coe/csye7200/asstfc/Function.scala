...
object Function {
  ...
  /**
    ...
    */
  // You know this one
  def lift[T, R](f: T => R): Try[T] => Try[R] = t1y => t1y.map(f)
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = (t1y, t2y) => map2(t1y, t2y)(f)
  ...
  def uncurried7[T1, T2, T3, T4, T5, T6, T7, T8, R](f: T1 => T2 => T3 => T4 => T5 => T6 => T7 => T8 => R): (T1, T2, T3, T4, T5, T6, T7) => T8 => R =
    (v1, v2, v3, v4, v5, v6, v7) => v8 => f(v1)(v2)(v3)(v4)(v5)(v6)(v7)(v8)
